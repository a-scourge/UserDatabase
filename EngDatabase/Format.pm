...
our @EXPORT_OK = qw(print_changes compare_hash parse_tcb parse_reg parse_grp add_propagation);
...
sub parse_tcb {
    ...
    return if ($gid<1000 && $id !~ m/^(webadmin|webuser|dnsmaint|cvsuser|eximuser)$/);
    ...
    foreach my $key ( sort keys %$db_href ) {
        ...
        if (ref($db_href->{$key}) eq 'ARRAY'){
            ...
            if ($existing_record->{$key} ne $db_href->{$key}) {
                my ($old_rec, $new_rec) = ($existing_record->{$key} , $db_href->{$key});
                #$changes->{$key} = "$old_rec, $new_rec";
                printf "%-10s Change %-15s from %10s to %10s\n", $username, $key, $old_rec, $new_rec
                    and $changed = 1;
            }
        }
        sub compare_array {
           ...
           if ($new->[0]{ATTRIBUTE_EFFECTIVE_DATE}) { 
               @{$new} = sort { $a->{ATTRIBUTE_EFFECTIVE_DATE} <=>
                $b->{ATTRIBUTE_EFFECTIVE_DATE} } @{$new};
               @{$old} = sort { $a->{ATTRIBUTE_EFFECTIVE_DATE} <=>
                $b->{ATTRIBUTE_EFFECTIVE_DATE} } @{$old};
            }
           if ($new->[0]{PRIMARY_GROUP}) { 
               @{$new} = sort { $a->{PRIMARY_GROUP} <=>
                $b->{mygroup}{GID} } @{$new};
               @{$old} = sort { $a->{PRIMARY_GROUP} <=>
                $b->{mygroup}{GID} }  @{$old};
            }
            for my $i (0 .. $#{$new}){
