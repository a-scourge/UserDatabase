...
-- Created on Fri Nov 15 16:19:30 2013
-- 
...
CREATE TABLE `PP_USERS` (
  ...
  `PRIMARY_GROUP` integer(11) NULL,
  `AFFILIATION_GROUP` integer(11) NULL,
  INDEX `PP_USERS_idx_PRIMARY_GROUP` (`PRIMARY_GROUP`),
  INDEX `PP_USERS_idx_STATUS_ID` (`STATUS_ID`),
  PRIMARY KEY (`USER_ID`),
  UNIQUE `both` (`ENGID`, `CRSID`),
  CONSTRAINT `PP_USERS_fk_PRIMARY_GROUP` FOREIGN KEY (`PRIMARY_GROUP`) REFERENCES `PP_GROUPS` (`GROUP_ID`) ON DELETE CASCADE ON UPDATE CASCADE,
