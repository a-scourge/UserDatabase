...
-- Created on Fri Nov 15 13:50:46 2013
-- 
...
CREATE TABLE `PP_GROUP_MEMBERSHIPS` (
  ...
  `USER_ID` integer(11) NOT NULL,
  `GROUP_ID` integer(11) NOT NULL,
  `PRIMARY_GROUP` integer(1) NULL,
  `AFFILIATION_GROUP` integer(1) NULL,
  ...
  UNIQUE `affiliationgroup` (`GROUP_MEMBERSHIP_ID`, `AFFILIATION_GROUP`),
  UNIQUE `both` (`GROUP_MEMBERSHIP_ID`, `AFFILIATION_GROUP`, `PRIMARY_GROUP`),
  UNIQUE `primarygroup` (`GROUP_MEMBERSHIP_ID`, `PRIMARY_GROUP`),
  ...
  CONSTRAINT `PP_GROUP_MEMBERSHIPS_fk_USER_ID` FOREIGN KEY (`USER_ID`) REFERENCES `PP_USERS` (`USER_ID`) ON DELETE CASCADE
) ENGINE=InnoDB;
