...
CREATE TEMPORARY TABLE PP_GROUP_MEMBERSHIPS_temp_alter (
  ...
  FOREIGN KEY (USER_ID) REFERENCES PP_USERS(USER_ID) ON DELETE CASCADE
);
...
CREATE UNIQUE INDEX affiliationgroup03 ON PP_GROUP_MEMBERSHIPS (GROUP_MEMBERSHIP_ID, AFFILIATION_GROUP);

CREATE UNIQUE INDEX both0203 ON PP_GROUP_MEMBERSHIPS (GROUP_MEMBERSHIP_ID, AFFILIATION_GROUP, PRIMARY_GROUP);

CREATE UNIQUE INDEX primarygroup03 ON PP_GROUP_MEMBERSHIPS (GROUP_MEMBERSHIP_ID, PRIMARY_GROUP);
